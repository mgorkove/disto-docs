...
Follow the instructions [here](user-pool) to create a user pool.
...
- `sagemaker-credentials-profile-name`: The name of the profile in the ~/.aws/credentials or ~/.aws/config files, which has either access keys or role information specified.
...
3. Run the aws-deploy script
   ...
   ./aws-deploy.sh \
    ...
    --sagemaker-credentials-profile-name "your_sagemaker_credentials_profile_name" \
